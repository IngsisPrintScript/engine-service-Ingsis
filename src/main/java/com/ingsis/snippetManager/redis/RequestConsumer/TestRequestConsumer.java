...
public class TestRequestConsumer extends RedisStreamConsumer<String> {
    ...
    public TestRequestConsumer(
            @Value("${redis.streams.runRequest}") String streamName,
            @Value("${redis.groups.run}") String groupName,
            RedisTemplate<String, String> redisTemplate,
            SnippetRunnerService service,
            TestResultProducer runResultProducer,
    ...
    public void onMessage(@NotNull ObjectRecord<String, String> record) {
        executor.submit(() -> {
            ...
            try {
                ...
                SnippetTestStatus finalStatus = success
                        ? SnippetTestStatus.PASSED
                ...
                if (event != null) {
                    snippetStatusService.markTestFailed(
                            event.snippetId(),
    ...
    private void publishWithRetry(
            ...
            UUID snippetId,
            UUID testId,
            SnippetTestStatus status) {
        for (int i = 1; i <= 3; i++) {
            try {
                runResultProducer.publish(
                        new TestResultEvent(ownerId,testId, snippetId,status)
                );
                return;
            } catch (Exception e) {
    ...
    public @NotNull StreamReceiver.StreamReceiverOptions<String, ObjectRecord<String, String>> options() {
        return StreamReceiver.StreamReceiverOptions
                .builder()
